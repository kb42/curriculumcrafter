...
# @app.route('/api/students', methods=['GET'])
# def get_students():
#     return jsonify(execute_query("SELECT * FROM Student"))

# @app.route('/api/courses', methods=['GET'])
# def get_courses():
#     return jsonify(execute_query("SELECT * FROM Course_Catalog"))
...
#     FROM AP_Credits
#     WHERE UPPER(CourseName)=UPPER(?) AND Score<=?
#     ORDER BY Score DESC
#     LIMIT 1
#     '''
#     result = execute_query(query, (course_name, score), one=True)
#     if result:
#         return jsonify({"CourseID": result['CourseID']})
#     return jsonify({"message": "No course credit available for this AP score"}), 404

# @app.route('/api/ap-credits/debug', methods=['GET'])
# def debug_ap_credits():
#     return jsonify(execute_query("SELECT * FROM AP_Credits"))

# @app.route('/api/ap-courses', methods=['GET'])
# def get_unique_ap_courses():
#     return jsonify(execute_query("SELECT DISTINCT CourseName FROM AP_Credits ORDER BY CourseName"))

# @app.route('/api/student/<netid>/plans', methods=['GET'])
# def get_student_plans(netid):
#     plans = execute_query(
#         "SELECT PlanID, CreationDate, NetID FROM Academic_Plan WHERE LOWER(NetID)=LOWER(?)", (netid,)
#     )
#     if not plans:
#         return jsonify({"error": "No academic plans found for this student"}), 404
#     return jsonify(plans)
...
#     WHERE pc.PlanID=?
#     '''
#     return jsonify(execute_query(query, (planid,)))

# @app.route('/api/majors', methods=['GET'])
# def get_majors():
#     return jsonify(execute_query("SELECT DISTINCT MajorID FROM Requirement"))
...
#     WHERE p.CourseID=?
#     '''
#     return jsonify(execute_query(query, (courseid,)))

# @app.route('/api/create-account', methods=['POST'])
# def create_account():
#     data = request.get_json()
#     name, netid, major, graduation = data.get('name'), data.get('netid'), data.get('majorid'), data.get('egrad')
#     if not all([name, netid, major, graduation]):
#         return jsonify({"error": "All fields are required"}), 400
#     if execute_query("SELECT 1 FROM Student WHERE NetID=?", (netid,), one=True):
#         return jsonify({"error": "Account with this NetID already exists"}), 409
#     execute_query(
#         "INSERT INTO Student(NetID,Name,Expected_Graduation,MajorID) VALUES(?,?,?,?)",
#         (netid, name, graduation, major), commit=True
#     )
#     return jsonify({"message": "Account created successfully"}), 201
...
# def login():
#     data = request.get_json()
#     netid, name = data.get('netid'), data.get('name')
#     if not all([netid, name]):
#         return jsonify({"error": "NetID and Name are required"}), 400
#     user = execute_query(
#         "SELECT * FROM Student WHERE NetID=? AND Name=?", (netid, name), one=True
#     )
#     if user:
#         return jsonify({"message": "Login successful!", "user": user}), 200
#     return jsonify({"error": "Invalid credentials"}), 401
...
# @app.route('/api/courses/search', methods=['GET'])
# def search_courses():
#     term = f"%{request.args.get('q','').lower()}%"
#     return jsonify(execute_query(
#         "SELECT CourseID FROM Course_Catalog WHERE LOWER(CourseID) LIKE ? LIMIT 50", (term,)
#     ))

# @app.route('/api/course/<courseid>/prerequisite-graph', methods=['GET'])
# def get_prerequisite_graph(courseid):
#     def fetch_prereqs(cid, visited):
#         if cid in visited:
#             return []
#         visited.add(cid)
#         rows = execute_query(
#             "SELECT PrerequisiteID FROM Prerequisite WHERE CourseID=?", (cid,)
#         )
#         edges = [{"from": r['PrerequisiteID'], "to": cid} for r in rows]
#         for r in rows:
#             edges += fetch_prereqs(r['PrerequisiteID'], visited)
#         return edges
#     visited, edges = set(), fetch_prereqs(courseid, set())
#     nodes = [{"id": c, "label": c} for c in visited]
...
#         "SELECT * FROM Student WHERE NetID=?", (netid,), one=True
#     )
#     if not existing:
#         return jsonify({"error": "Account does not exist"}), 404
#     name, major, grad = data.get('name'), data.get('majorid'), data.get('egrad')
#     execute_query(
#         "UPDATE Student SET Name=?, MajorID=?, Expected_Graduation=? WHERE NetID=?",
#         (name or existing['Name'], major or existing['MajorID'], grad or existing['Expected_Graduation'], netid),
#         commit=True
#     )
#     return jsonify({"message": "Information updated successfully"}), 200
...
# @app.route('/api/plan', methods=['POST'])
# def add_plan():
#     netid = request.get_json().get('netid')
#     if not netid:
#         return jsonify({"error": "NetID is required"}), 400
#     result = execute_query("SELECT MAX(PlanID) as maxid FROM Academic_Plan", one=True)
#     new_id = (result['maxid'] or 0) + 1
#     execute_query(
#         "INSERT INTO Academic_Plan(PlanID,CreationDate,NetID) VALUES(?,date('now'),?)",
#         (new_id, netid), commit=True
#     )
#     return jsonify({"message": f"Plan added with PlanID {new_id}", "planid": new_id}), 201

# @app.route('/api/course', methods=['POST'])
# def add_course():
#     data = request.get_json()
#     planid, courseid, semester = data.get('planid'), data.get('courseid','').strip().upper(), data.get('semester','').strip().upper()
#     if not all([planid, courseid, semester]):
#         return jsonify({"error": "planid, courseid, semester are required"}), 400
#     if not execute_query("SELECT 1 FROM Academic_Plan WHERE PlanID=?", (planid,), one=True):
#         return jsonify({"error": f"PlanID {planid} does not exist"}), 404
#     if not execute_query("SELECT 1 FROM Course_Catalog WHERE CourseID=?", (courseid,), one=True):
#         return jsonify({"error": f"CourseID {courseid} does not exist"}), 404
#     if execute_query("SELECT 1 FROM Planned_Course WHERE PlanID=? AND CourseID=?", (planid, courseid), one=True):
#         return jsonify({"error": f"Course {courseid} already in plan"}), 409
#     execute_query(
#         "INSERT INTO Planned_Course(PlanID,CourseID,Semester) VALUES(?,?,?)",
#         (planid, courseid, semester), commit=True
#     )
#     return jsonify({"message": f"Course {courseid} added to PlanID {planid}"}), 201
...
# def delete_course(planid, courseid):
#     execute_query(
#         "DELETE FROM Planned_Course WHERE PlanID=? AND CourseID=?",
#         (planid, courseid), commit=True
#     )
#     return jsonify({"message": f"Course {courseid} removed from PlanID {planid}"}), 200
...
#     FROM Planned_Course pc
#     JOIN Course_Catalog cc ON pc.CourseID=cc.CourseID
#     WHERE pc.PlanID=? AND UPPER(pc.Semester)=UPPER(?)
#     '''
#     result = execute_query(query, (planid, semester), one=True)
#     return jsonify({'total_credits': result['total_credits'] if result else 0})

# @app.route('/api/course/prerequisites/<courseid>', methods=['GET'])
# def check_prerequisites(courseid):
#     rows = execute_query("SELECT PrerequisiteID FROM Prerequisite WHERE CourseID=?", (courseid,))
#     return jsonify({'missingPrerequisites': [r['PrerequisiteID'] for r in rows]})

# @app.route('/api/course/add-with-bypass', methods=['POST'])
# def add_course_with_bypass():
#     data = request.get_json()
#     planid, courseid, semester, bypass_reason, netid = (
#         data.get('planid'), data.get('courseid'), data.get('semester'), data.get('bypassReason'), data.get('netid')
#     )
#     # credit checks
#     credit_q = '''
#     SELECT COALESCE(SUM(cc.Credits),0) as current_credits,
#            (SELECT Credits FROM Course_Catalog WHERE CourseID=?) as new_credits
#     FROM Planned_Course pc
#     JOIN Course_Catalog cc ON pc.CourseID=cc.CourseID
#     WHERE pc.PlanID=? AND UPPER(pc.Semester)=UPPER(?)
#     '''
#     res = execute_query(credit_q, (courseid, planid, semester), one=True)
#     if res['current_credits']+res['new_credits']>18:
#         return jsonify({'error': 'Exceeds max credits'}), 400
#     count = execute_query(
#         "SELECT COUNT(*) as cnt FROM Planned_Course WHERE PlanID=? AND UPPER(Semester)=UPPER(?)",
#         (planid, semester), one=True
#     )
#     if count['cnt']>=6:
#         return jsonify({'error': 'Exceeds max courses'}), 400
#     # log bypass
#     execute_query(
#         "INSERT INTO Prerequisite_Bypass_Log(NetID,CourseID,PlanID,BypassReason,BypassDate) VALUES(?,?,?,?,date('now'))",
#         (netid, courseid, planid, bypass_reason), commit=True
#     )
#     # add course
#     execute_query(
#         "INSERT INTO Planned_Course(PlanID,CourseID,Semester) VALUES(?,?,?)",
#         (planid, courseid, semester), commit=True
#     )
#     return jsonify({'message': 'Course added with bypass'}), 201
...
#     if planid:
#         q = '''
#         SELECT ap.PlanID, s.NetID, s.Name, COALESCE(SUM(cc.Credits),0) as Total
...
#         WHERE s.NetID=? AND ap.PlanID=?
#         GROUP BY ap.PlanID,s.NetID,s.Name
#         '''
#         res = execute_query(q, (netid, planid), one=True)
#         if res:
#             return jsonify({"netid": res['NetID'], "total_credits": res['Total'], "message": f"Hey {res['NetID']}, you have {res['Total']} credits!"})
#     return jsonify({"netid": netid, "total_credits": 0, "message": f"Hey {netid}, no credits planned yet."})

# @app.route('/api/students/requirements/<netid>', methods=['GET'])
# def find_students_fulfilling_requirements(netid):
#     student = execute_query("SELECT MajorID FROM Student WHERE NetID=?", (netid,), one=True)
#     if not student:
...
#         ''', (netid,)
#     )
#     all_req = execute_query("SELECT CourseID,Credits FROM Requirement WHERE MajorID=?", (major_id,))
#     fulfilled_ids = {r['CourseID'] for r in fulfilled}
#     unfulfilled = [c for c in all_req if c['CourseID'] not in fulfilled_ids]
#     return jsonify({"majorId": major_id, "fulfilled": fulfilled, "unfulfilled": unfulfilled})
